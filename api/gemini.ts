...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (isRateLimited) {
      console.warn("[Notice] Rate limited generating proxy content. Using fallback.");
      ...
      console.warn("[Notice] Gemini API unavailable. Applying fallback: ", error?.message || error);
    }
    return res.status(isRateLimited ? 429 : 500).json({ error: 'Failed to generate content', details: error.message });
  }
}
